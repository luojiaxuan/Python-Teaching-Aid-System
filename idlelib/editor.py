...
class EditorWindow(object):
    ...
    pythonwork_name = "D:\\毕业设计3-13\\idlelib\\test1.py"
    ...
    def __init__(self, flist=None, filename=None, key=None, root=None):
        ...
        self.pythonwork_tree = ttk.Treeview(text_frame, columns=("work-num", "state"),show="headings")
        self.pythonwork_tree.column("work-num",width=100,anchor="center")
        self.pythonwork_tree.column("state", width=100, anchor="center")
        self.pythonwork_tree.heading("work-num",text="pythonwork")
        self.pythonwork_tree.heading("state", text="workstate")
        self.pythonwork_filelist = []
        # modify by luojiaxuan
        import os
        getmydir = os.getcwd()
        self.pythonwork_file = getmydir+"\\pythonworkfile.txt"
        self.load_file_pythonwork_info()
        self.load_file_to_treeview(self.pythonwork_filelist)
        self.pythonwork_tree.bind("<Double-1>",self.load_file_to_textframe)
        self.pythonwork_tree.grid(row=0,column=0,sticky=NSEW,rowspan=4) # 设置继承类MWindow的grid布局位置，并向四个方向拉伸以填充顶级窗体
        # self.pythonwork_tree.rowconfigure(0, weight=1) # 设置继承类MWindow的行列权重，保证内建子组件会拉伸填充
        self.pythonwork_tree.columnconfigure(0, weight=1)


        self.user_label = Label(text_frame,text="XX，欢迎你！")
        self.user_label.grid(row=0,column=1,sticky=NSEW)

        self.button_frame = Frame(text_frame)

        scriptbinding = ScriptBinding(self)
        self.run_button = Button(text_frame,text="运行")
        self.run_button.bind("<Button-1>",scriptbinding.run_module_event) # 这里是将鼠标左键和运行按钮绑定
        self.run_button.grid(row=3,column=2,sticky=NSEW)

        self.keep_button = Button(text_frame, text="保存",command=self.pythonwork_keep) # 说明Button的click事件会先触发并调用事件处理函数， 接着调用Command的处理。
        self.keep_button.bind("<Button-1>", self.pythonwork_keep)
        self.keep_button.grid(row=3,column=4,sticky=NSEW)

        self.run_button = Button(text_frame, text="提交")
        self.run_button.bind("<Button-1>", self.pythonwork_commit)  # 这里是将鼠标左键和运行按钮绑定
        self.run_button.grid(row=3, column=6, sticky=NSEW)

        self.debuger_text = Text(text_frame)
        self.debuger_text.grid(row=2,column=1,columnspan=6)
        ...
        vbar.grid(row=1, column=7, sticky=NSEW)
        text['yscrollcommand'] = vbar.set
        text['font'] = idleConf.GetFont(self.root, 'main', 'EditorWindow')
        text.grid(row=1, column=1, sticky=NSEW,columnspan=6)
        text.focus_set()
        self.set_width()
    ...
    def load_file_pythonwork_info(self):
        if not os.path.exists(self.pythonwork_file):
            print("不存在文件")
        else:
            try:
                with open(file=self.pythonwork_file,mode="r",encoding="UTF-8") as fd:
                    current_line = fd.readline()
                    while current_line:
                        temp_list = current_line.split(",")
                        print(temp_list)
                        self.pythonwork_filelist.append(temp_list)
                        current_line = fd.readline()
            except:
                print("文件读取异常")

    def load_file_to_treeview(self,current_list:list):
        if len(current_list)==0:
            print("没有数据加载")
        else:
            for index in range(len(current_list)):
                self.pythonwork_tree.insert("",index,values=(
                    self.pythonwork_filelist[index][0],self.pythonwork_filelist[index][1]
                ))

    def load_file_to_textframe(self,event):

        # 先将界面清空
        self.text.delete("iomark", "end")

        # 获取双击行的数据
        item_1 = self.pythonwork_tree.focus()
        item_2 = self.pythonwork_tree.item(item_1,"values")[0]


        self.pythonwork_name = "C:\\Users\\zhaos\\AppData\\Local\\Programs\\Python\\Python37\\Lib\\idlelib\\"+item_2+".py"
        print("更改执行文件名:"+self.pythonwork_name)
        self.pythonwork_file = self.pythonwork_name
        if not os.path.exists(self.pythonwork_file):
            print("不存在文件")
        else:
            try:
                with open(file=self.pythonwork_file, mode="r", encoding="UTF-8") as fd:
                    current_line = fd.readline()
                    while current_line:
                        self.text.insert("insert",current_line)
                        current_line = fd.readline()
            except:
                print("文件读取异常")

    def pythonwork_keep(self,event):
        print("保存的py文件为："+self.pythonwork_name)
        if not os.path.exists(self.pythonwork_name):
            print("不存在文件")
        else:
            try:
                with open(file=self.pythonwork_name,mode="w") as fd:
                    fd.write(self.text.get(1.0, END))
            except:
                print("文件读取异常")

    def pythonwork_commit(self,event):
        pass

    def get_pythonwork_name(self):
        return self.pythonwork_name
    ...
    def open_module(self):
        """Get module name from user anden it.
