...
class PyShellFileList(FileList):
    ...
    def open_shell(self, username=None,event=None):
        if self.pyshell:
            self.pyshell.top.wakeup()
        else:
            self.pyshell = PyShell(self)
            if self.pyshell:
                if not self.pyshell.begin(username):
                    return None
        return self.pyshell
...
class PyShell(OutputWindow):
    ...
    def begin(self,username=None):
        ...
        #self.write("Python %s on %s\n%s\n%s" % (sys.version, sys.platform, self.COPYRIGHT, nosub))
        self.write("")
        # modify by luojiaxuan
        #self.user_label["text"] = username+"，欢迎你！"
        print("是否通过断点12")
    ...
    def write(self, s, tags=()):
        try:
            ...
            # modify by luojiaxuan
            #self.debuger_text.insert("insert",s)
...
def main(username=None):
    ...
    if enable_shell:
        shell = flist.open_shell(username)
        if not shell:
