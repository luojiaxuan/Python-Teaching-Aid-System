...
class ScriptBinding:
    ...
    def __init__(self, editwin):
        ...
        self.filename = self.editwin.get_pythonwork_name()

        print("传进来的文件名是？"+self.filename)
        # self.stderr = self.editwin.stderr
    ...
    def check_module_event(self, event):
        ...
        filename = self.editwin.get_pythonwork_name()
        if not filename:
            return 'break'
    ...
    def _run_module_event(self, event, *, customize=False):
        ...
        #         转到__stderr__。使用子进程，它们会进入外壳。
        #         需要在pyshell.ModifiedInterpreter中更改流。
        interp.runcode(code)
        return 'break'

    def getfilename(self):
        """获取源文件名。如果未保存，请提供保存(或创建)文件。
